...
3. **Details** (`GlassCard`): name input · description · **meal type** (`Segmented`, 4) · **Servings / Prep / Cook** steppers · **Tags** chips (removable) + "Add tags" → `MultiSelectDialog(DIETS)`.
...
## Reuse
- `components/`: `NavBar`, `GlassCard`, `Segmented`, `MultiSelectDialog` (tags), `ConfirmDialog` (remove), `PhoneFrame`, `StatusBar`, tokens. Header/footer/stepper recipes mirror `plan-builder` / `workout-builder`. Macro colors + bar pattern mirror `libraries/MealPreviewScreen`.
