...
// Container-transform bottom sheet — scrim fades + sheet rises (0.32s standard ease).
// `shown` flips to true on mount via rAF so the entry transition plays.
function BottomSheet({ onDismiss, dismissProp = 'onClick', style: extra, children }) {
  const [shown, setShown] = useState(false)
  useEffect(() => { const id = requestAnimationFrame(() => setShown(true)); return () => cancelAnimationFrame(id) }, [])
  const dismiss = { [dismissProp]: onDismiss }
  return (
    <>
      <div {...dismiss} style={{ position: 'absolute', inset: 0, zIndex: 40, background: 'rgba(var(--cs-shadow-rgb),0.55)', backdropFilter: 'blur(2px)', WebkitBackdropFilter: 'blur(2px)', opacity: shown ? 1 : 0, transition: `opacity 0.32s ${EASE}` }} />
      <div style={{ ...sheetWrapSt, ...extra, transform: shown ? 'translateY(0)' : 'translateY(100%)', transition: `transform 0.32s ${EASE}` }}>
        {children}
      </div>
    </>
  )
...
// Container-transform dialog scrim — scrim fades + dialog scales/fades up (0.32s standard ease).
function DialogScrim({ onDismiss, children }) {
  const [shown, setShown] = useState(false)
  useEffect(() => { const id = requestAnimationFrame(() => setShown(true)); return () => cancelAnimationFrame(id) }, [])
  return (
    <div onClick={onDismiss} style={{ position: 'absolute', inset: 0, zIndex: 80, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, background: 'rgba(var(--cs-shadow-rgb),0.55)', opacity: shown ? 1 : 0, transition: `opacity 0.32s ${EASE}` }}>
      <div onClick={e => e.stopPropagation()} style={{ width: '100%', display: 'flex', justifyContent: 'center', transform: shown ? 'scale(1)' : 'scale(0.92)', opacity: shown ? 1 : 0, transition: `transform 0.32s ${EASE}, opacity 0.32s ${EASE}` }}>
        {children}
      </div>
    </div>
  )
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column' }}>
        <NavBar>
          ...
          <div style={{ display: 'flex', alignItems: 'center', padding: '4px 16px 12px', gap: 8 }}>
            ...
            <span style={{ ...TT, flex: 1, fontSize: 16, fontWeight: 500, color: 'var(--cs-on-surface)', textAlign: 'center' }}>
              {initialStep === 'empty' ? 'New Plan' : 'Edit Plan'}
            </span>
            <IconButton size="md" icon={<CheckIcon />} aria-label="Save plan"
              style={{ background: 'var(--cs-primary)', border: 'none', color: 'var(--cs-on-primary)', opacity: M.validatePlan(plan) ? 1 : 0.4 }} />
          </div>
        </NavBar>
        ...
        {/* page-action footer — FabMenu carries the page-level Save */}
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '12px 16px 28px', background: 'linear-gradient(0deg, rgba(var(--cs-surface-rgb),0.92) 55%, transparent)' }}>
          <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
            <FabMenu open={fabOpen} setOpen={setFabOpen} actions={[
              { label: 'Add to current day', icon: <PlusIcon size={15} />, onClick: () => { if (expanded) on.addWorkout(expanded.week, expanded.day) } },
              { label: 'Save plan', icon: <CheckIcon />, primary: true, dividerAbove: true, onClick: () => { /* stub — save */ } },
            ]} />
          </div>
        </div>
        ...
        {/* ── Week actions menu ── */}
        {weekMenu != null && (
          <BottomSheet onDismiss={() => setWeekMenu(null)} style={{ padding: '10px 12px 26px' }}>
              ...
              <p style={{ ...TT, fontSize: 16, fontWeight: 500, color: 'var(--cs-on-surface)', margin: '0 4px 8px', padding: '0 4px' }}>Week {weekMenu + 1}</p>
              {[
        ...
        {dayMenu && (() => {
          ...
          return (
            <BottomSheet onDismiss={() => setDayMenu(null)} dismissProp="onPointerDown" style={{ padding: '10px 12px 26px' }}>
                <div style={{ display: 'flex', justifyContent: 'center', paddingBottom: 8 }}><div style={{ width: 36, height: 4, borderRadius: 2, background: 'rgba(var(--overlay-rgb),0.16)' }} /></div>
                <p style={{ ...TT, fontSize: 16, fontWeight: 500, color: 'var(--cs-on-surface)', margin: '0 4px 8px', padding: '0 4px' }}>
        ...
        {/* ── Clear-day confirm ── */}
        {clearAsk && (
          <DialogScrim onDismiss={() => setClearAsk(null)}>
            <ConfirmDialog
...
function CopyTargetsSheet({ plan, src, onCancel, onConfirm }) {
  ...
  return (
    <BottomSheet onDismiss={onCancel} style={{ maxHeight: '80%', display: 'flex', flexDirection: 'column', padding: '10px 14px 18px' }}>
        ...
        <p style={{ ...TT, fontSize: 16, fontWeight: 500, color: 'var(--cs-on-surface)', margin: '0 2px 2px' }}>Copy {DAY_FULL[src.day]} · Week {src.week + 1}</p>
        ...
        <div style={{ flex: 1, overflowY: 'auto', minHeight: 0 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '30px repeat(7, 1fr)', columnGap: 4, marginBottom: 6 }}>
            <span />
            {M.DAYS.map(d => <span key={d} style={{ ...TT, fontSize: 9, fontWeight: 500, letterSpacing: '0.04em', textTransform: 'uppercase', color: 'var(--cs-on-surface-variant)', opacity: 0.4, textAlign: 'center' }}>{d}</span>)}
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            {plan.schedule.map((week, w) => (
              <div key={w} style={{ display: 'grid', gridTemplateColumns: '30px repeat(7, 1fr)', columnGap: 4, alignItems: 'center' }}>
                <span style={{ ...TT, fontSize: 10, fontWeight: 500, color: 'var(--cs-on-surface-variant)', opacity: 0.7, textAlign: 'center' }}>W{w + 1}</span>
                {week.map((cell, d) => {
                  ...
                  return (
                    <button key={d} onClick={() => toggle(w, d)} disabled={me} style={{
                      ...
                      {info
                        ? <span style={{ fontSize: 10, fontWeight: 500, color: (me || on) ? 'var(--cs-primary)' : `rgba(var(${info.color}),1)` }}>{FOCUS_ABBR[info.focus] ?? info.focus.slice(0, 2)}</span>
                        : <span style={{ width: 3, height: 3, borderRadius: '50%', background: on ? 'var(--cs-primary)' : 'rgba(var(--cs-primary-rgb),0.3)' }} />}
                      {me && <span style={{ position: 'absolute', top: 1, right: 3, ...TT, fontSize: 7, fontWeight: 500, color: 'var(--cs-primary)' }}>from</span>}
                    </button>
                  )
                })}
              </div>
