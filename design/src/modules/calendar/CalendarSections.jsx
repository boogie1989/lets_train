// Calendar sections — the phone screen's building blocks, lifted verbatim from
// CalendarScreen.jsx so the tablet / desktop variants can re-compose them.
// RULE: these are shared pixels — never fork styles per screen; the phone
// screen stays the visual reference for every block here.
...
import { DaySummary, MonthGrid, ReadinessCard } from './CalendarWidgets.jsx'

const TT = { fontFamily: 'var(--tt-font-family)' }
export const USER = { name: 'Serhii Buhai', email: 'serhii.work@gmail.com', initials: 'SB' }
...
export const blockGroup = { display: 'flex', flexDirection: 'column', gap: 8 }
const sectionLabel = {
  ...TT,
  paddingLeft: 2,
  fontSize: 11,
...
// ── Shared calendar state — one hook for phone / tablet / desktop ────────────
export function useCalendarState({ initialDay = TODAY, initialMonthOpen = false, initialDetailId = null } = {}) {
  ...
  const shiftMonth = dir => setMonthOffset(o => Math.max(-1, Math.min(1, o + dir)))
  ...
  const fabActions = [
    { label: 'Schedule workout', icon: <BarbellGlyph />, onClick: () => setMonth(m => addItem(m, selected, { kind: 'workout', title: 'Quick Workout', time: '05:00 PM', exerciseCount: 6, status: 'Planned' })) },
    { label: 'Schedule meal', icon: <MealGlyph />, onClick: () => setMonth(m => addItem(m, selected, { kind: 'meal', title: 'Snack', time: '04:00 PM', kcal: 280, p: 18, c: 30, f: 9, status: 'Planned' })) },
    { label: 'Log meal', icon: <LoggedMealGlyph />, dividerAbove: true, onClick: () => setMonth(m => addItem(m, selected, { kind: 'meal', title: 'Snack', time: '04:00 PM', kcal: 280, p: 18, c: 30, f: 9, status: 'Completed' })) },
  ]
  ...
  return {
    ...
    monthOffset, setMonthOffset, shiftMonth,
    fabOpen, setFabOpen,
    menuOpen, setMenuOpen,
    lib, setLib,
    detailId, setDetailId, detailItem, detailHandlers,
    snack, undo,
    day, items, stats, nut, dayLoad, isToday, isPast, dayCompleted,
    setReadiness, fabActions,
  }
}

// ── Top bar — grid (1fr auto 1fr) keeps the title optically centered
//    regardless of whether the right-side Today button is present.
//    monthToggle=false renders the month as a static label; showMonth=false
//    drops it entirely and showToday=false drops the Today pill (big screens
//    carry both in the MonthHeader above the calendar instead). ──────────────
export function CalendarTopBar({ onMenu, showMenu = true, monthToggle = true, showMonth = true, showToday = true, monthOpen, monthOffset = 0, onToggleMonth, isToday, onToday }) {
  return (
    <div style={{ height: 56, display: 'grid', gridTemplateColumns: '1fr auto 1fr', alignItems: 'center' }}>
      <div style={{ justifySelf: 'start' }}>
        {showMenu && (
          <button onClick={onMenu} style={{ ...iconBtn, cursor: 'pointer' }} aria-label="Open menu">
            <MenuIcon />
          </button>
        )}
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
        ...
        {showMonth && (monthToggle ? (
          <MonthTogglePill open={monthOpen} monthOffset={monthOffset} onClick={onToggleMonth} />
        ) : (
          <span style={{
            ...TT, padding: '3px 10px',
      ...
      <div style={{ justifySelf: 'end' }}>
        {showToday && !isToday && <TodayPill onClick={onToday} />}
      </div>
    </div>
  )
}

// ── Month header — sits ABOVE the standalone big-screen calendar card ────────
// Month label left; right cluster: Today pill (shown when the selection is off
// today or a ghost month is paged in) + ‹ › month pagers. Paging shows the
// ghost neighbours (April / June, no demo data) exactly like the phone's
// month-view swipe; the offset is clamped to ±1, so the pagers dim at the
// bounds. Selecting a day resets the offset.
export function MonthHeader({ monthOffset = 0, onShiftMonth, isToday, onToday }) {
  const label = monthOffset !== 0 ? NEIGHBOR_MONTHS[monthOffset].label : MONTH_LABEL
  const pager = dir => {
    const disabled = monthOffset === dir
    return (
      <button
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      <span style={{ ...TT, fontSize: 15, fontWeight: 500, color: 'var(--cs-on-surface)' }}>
        {label}
      </span>
      <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 6 }}>
        {(!isToday || monthOffset !== 0) && <TodayPill onClick={onToday} />}
        {pager(-1)}
        {pager(1)}
      </div>
    </div>
...
// Month label = week/month toggle (chevron flips when open); the faint pill
// backing marks it as tappable without shouting
export function MonthTogglePill({ open, monthOffset = 0, onClick }) {
  return (
    <button onClick={onClick} style={{
      ...
      {open && monthOffset !== 0 ? NEIGHBOR_MONTHS[monthOffset].label : MONTH_LABEL}
      <span style={{ display: 'flex', transform: open ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s', opacity: 0.7 }}>
        <ChevDown />
      </span>
    </button>
  )
...
export function TodayPill({ onClick }) {
  return (
    <button onClick={onClick} aria-label="Jump to today" style={{
...
export function DateStrip({ month, selected, weekIdx, monthOpen, monthOffset, onSelectDay, onShiftWeek, onShiftMonth, dayCompleted }) {
  ...
  const swipeHandlers = {
    ...
    onPointerUp: () => {
      ...
      if (monthOpen) onShiftMonth(dir)
      else onShiftWeek(dir)
    },
  ...
  return (
    <div {...swipeHandlers} style={{ touchAction: 'pan-y', overflow: 'hidden', cursor: 'grab' }}>
      <div style={{
        ...
        {monthOpen ? (
          <MonthGrid month={month} selected={selected} onSelect={onSelectDay}
...
export function DateCaption({ weekday, dayN }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
      <span style={{
        ...
        {WEEKDAY_FULL[weekday]}, {dateLabel(dayN)}
      </span>
    </div>
  )
}
...
export function ReadinessBlock({ day, isToday, isPast, onSet }) {
  if (!(isToday || (isPast && day.readiness))) return null
  return (
    <div style={blockGroup}>
      <SectionLabel>How are you today?</SectionLabel>
      {isToday && <ReadinessCard value={day.readiness} onSet={onSet} />}
      {isPast && day.readiness && <ReadinessCard value={day.readiness} readOnly />}
    </div>
  )
}

export function SummaryBlock({ stats, load, nut }) {
  return (
    <div style={blockGroup}>
      <SectionLabel>Summary</SectionLabel>
      <DaySummary stats={stats} load={load} nut={nut} />
    </div>
  )
}

// ── Schedule — timeline (time gutter + status-node rail) or plain list ───────
export function ScheduleBlock({ items, isPast, timeline = true, onOpen }) {
  return (
    <div style={blockGroup}>
...
export function ScheduleEmpty() {
  return <EmptyState icon={<CalendarOffIcon />} title="No workouts scheduled" caption="Press + to add a workout" style={{ transform: 'translateY(-50%)' }} />
}

// ── Footer — FAB menu (shared FabMenu, same recipe as Workout Builder) ───────
export function FabFooter({ open, setOpen, actions, padding = '12px 16px 28px' }) {
  return (
    <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, zIndex: 40, padding, pointerEvents: 'none' }}>
      <div style={{ display: 'flex', justifyContent: 'flex-end', pointerEvents: 'auto' }}>
        <FabMenu open={open} setOpen={setOpen} actions={actions} />
      </div>
    </div>
  )
}

// ── Drawer / rail navigation ─────────────────────────────────────────────────
export const LIBS = [
...
// short profile + library navigation — the drawer's body, also promoted to the
// permanent desktop rail
export function NavPanelBody({ onNavigate, onProfile }) {
  return (
    <>
      {/* short profile */}
      <button onClick={onProfile} style={{ ...TT, display: 'flex', alignItems: 'center', gap: 12, padding: '8px 16px 16px', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left' }}>
...
// ── Side drawer: scrim + left slide-in panel around NavPanelBody ─────────────
export function SideDrawer({ open, onClose, onNavigate }) {
  return (
    <>
      ...
      <div style={{
        ...
        <NavPanelBody onNavigate={onNavigate} onProfile={onClose} />
      </div>
    </>
  )
}
...
export function LoggedMealGlyph() {
