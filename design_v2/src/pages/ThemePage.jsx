...
const COLOR_GROUPS = [
  ...
  {
    label: 'Surface elevation ladder (opaque · v2)',
    swatches: ['--surface-0', '--surface-1', '--surface-2', '--surface-3', '--surface-4'],
  },
  {
...
const GRADIENTS = [
  { name: 'screen background', var: '--bg-gradient',           desc: 'Static, baked screen background (no blur / no animation). → Flutter RadialGradient/LinearGradient' },
  { name: 'slateAccent',       var: '--gradient-slate-accent', desc: 'Optional accent fill (selected states default to flat --cs-primary)' },
]

// v2 elevation language: surface color + 1px border + ONE soft shadow.
const ELEV = [
  { name: 'card · elev-1', surface: '--surface-2', border: '--border-subtle',  shadow: '--elev-1' },
  { name: 'sheet · elev-2', surface: '--surface-3', border: '--border-default', shadow: '--elev-2' },
  { name: 'raised · elev-2', surface: '--surface-4', border: '--border-strong', shadow: '--elev-2' },
]
...
export default function ThemePage() {
  ...
  return (
    <div style={{ padding: '40px 48px', maxWidth: 960, margin: '0 auto' }}>
      ...
      {/* ── Borders & elevation ── */}
      <Section title="Borders & elevation" subtitle="separation = surface color + 1px border + ONE soft shadow · no glass / no inset · → Flutter Card(elevation)">
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16 }}>
          {ELEV.map((e) => (
            <div key={e.name} style={{
              width: 200, height: 96, borderRadius: 'var(--radius-2xl)',
              background: `var(${e.surface})`,
              border: `1px solid var(${e.border})`,
              boxShadow: `var(${e.shadow})`,
              display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: 12,
            }}>
              <p style={{ fontFamily: 'monospace', fontSize: 11, fontWeight: 500, color: 'var(--cs-on-surface)' }}>{e.name}</p>
              <p style={{ fontFamily: 'monospace', fontSize: 9, color: 'var(--cs-on-surface-variant)', marginTop: 2, opacity: 0.7 }}>{e.surface} · {e.border}</p>
            </div>
          ))}
        </div>
      </Section>
      ...
      {/* ── Gradients ── */}
      <Section title="Gradients" subtitle="--gradient-* · static only (no runtime blur)">
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16 }}>
          {GRADIENTS.map((g) => (
      ...
      {/* ── Tabular figures ── */}
      <Section title="Tabular figures" subtitle=".tnum · font-variant-numeric: tabular-nums · → Flutter FontFeature.tabularFigures()">
        <div style={{ display: 'flex', gap: 32, flexWrap: 'wrap' }}>
          {[['proportional (default)', false], ['tabular (.tnum)', true]].map(([label, tnum]) => (
            <div key={label}>
              <GroupLabel>{label}</GroupLabel>
              <div style={{
                fontFamily: 'var(--tt-font-family)', fontSize: 16, lineHeight: 1.6,
                color: 'var(--cs-on-surface)', textAlign: 'right', width: 160,
      ...
      {/* ── Density ── */}
      <Section title="Density" subtitle='--density-row / --density-gap · [data-density="compact"] flips comfortable↔compact'>
        <div style={{ display: 'flex', gap: 32, flexWrap: 'wrap' }}>
          {['comfortable', 'compact'].map((mode) => (
            <div key={mode} data-density={mode === 'compact' ? 'compact' : undefined}>
              <GroupLabel>{mode}</GroupLabel>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--density-gap)', width: 220 }}>
                {['Morning Strength', 'Leg Day', 'Evening Yoga'].map((t) => (
                  <div key={t} style={{
                    height: 'var(--density-row)', display: 'flex', alignItems: 'center', padding: '0 16px',
                    background: 'var(--surface-2)', border: '1px solid var(--border-subtle)',
                    borderRadius: 'var(--radius-2xl)', boxShadow: 'var(--elev-1)',
                    fontFamily: 'var(--tt-font-family)', fontSize: 14, fontWeight: 500, color: 'var(--cs-on-surface)',
                  }}>{t}</div>
                ))}
              </div>
            </div>
