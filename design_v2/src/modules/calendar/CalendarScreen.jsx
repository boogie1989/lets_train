...
// v2: opaque chrome slab — no backdrop blur. Separation = surface color +
// one hairline bottom border + a single soft shadow. → Flutter Material(elevation).
const cardSlab = {
  width: '100%',
...
// v2: filled-tonal icon button (→ Flutter IconButton.filledTonal)
const iconBtn = {
  width: 48, height: 48,
...
export default function CalendarScreen({ initialDay = TODAY, initialMonthOpen = false, initialDetailId = null }) {
  ...
  return (
    <PhoneFrame>
      {/* Calendar Card — unified opaque chrome slab, full-bleed from top, no border-radius */}
      <div style={cardSlab}>
        <StatusBar />
      ...
      {/* ── Side menu drawer ── (library nav is stubbed in v2 — calendar ships
          standalone; the Libraries module is not part of this first iteration) */}
      <SideDrawer open={menuOpen} onClose={() => setMenuOpen(false)} onNavigate={() => setMenuOpen(false)} />
    </PhoneFrame>
  )
}
  ...
  return (
    <>
      ...
      <div style={{
        ...
        background: 'var(--surface-3)',
        borderRight: '1px solid var(--border-default)', boxShadow: 'var(--elev-2)',
