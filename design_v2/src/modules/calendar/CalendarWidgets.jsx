...
const card = { flexShrink: 0, background: 'var(--surface-2)', borderRadius: 'var(--radius-2xl)', border: '1px solid var(--border-subtle)', boxShadow: 'var(--elev-1)' }
...
// numeric meta uses tabular figures so columns of kg/AU/kcal/min align
          ...
          {week.map((n, i) => {
            ...
            return (
              <button key={n} onClick={() => onSelect(n)} style={{
                ...
                // today = 1px primary ring. → Flutter ChoiceChip / custom paint.
                background: isSel ? 'var(--cs-primary)' : 'transparent',
                border: isToday && !isSel ? '1px solid rgba(var(--cs-primary-rgb),0.55)' : '1px solid transparent',
                boxShadow: isSel ? 'var(--elev-1)' : 'none',
              }}>
                <span style={{ fontVariantNumeric: 'tabular-nums', fontSize: 15, fontWeight: 'var(--tt-title-medium-weight)', lineHeight: 1, color: isSel ? 'var(--cs-on-primary)' : 'var(--cs-on-surface)' }}>{n}</span>
